    ...
    <table class="table ui-accordion">
      ...
      <tbody>
        <template v-for="row in data.rows">
          <tr class="head" :key="row.title.order" @click="onRowClick(row.title.order)">
            <td>{{ row.title.order }}</td>
            <td>{{ row.title.type }}</td>
            <td>{{ row.title.question }}</td>
          </tr>
          <tr class="body" :class="{ active: row.title.order === selectRow }" :key="`${row.title.order}_content}`" >
            <td colspan="3">
...
export default {
  ...
  methods: {
    onRowClick (i) {
      this.selectRow = this.selectRow === i ? 0 : i
    }
  }
}
</script>
